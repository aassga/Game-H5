      ...
      <ul class="lottery_vns">
          ...
          >
            ...
            <div class="game_title">
              ...
              <hall-time @update:closed='closeResult' :id="item.id" :result="result[i]" v-if="result[i] && item.name!=='长龙助手'"></hall-time>
...
<script>
import HallTime from "../hall-time/timer1"
...
export default {
  ...
  watch:{
    list(){
      ...
      for(let key in this.list) {
        if(this.list[key].id!==99996) idList.push(this.list[key].id)
      }
      this.idList = idList.join();
      console.log(idList)
      this.getTimeList(this.idList);
    },
  ...
  methods: {
    imgUrl (url) {
      if(url.indexOf('longDragon.png') > 0) {
        return url
      }else {
    ...
    getTimeList(id) {
        this.$store.dispatch('lottery/getLotteryTime', id).then(res => {
           console.log(res)
