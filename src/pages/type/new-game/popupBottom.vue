  ...
  >
    <div class="container theme-main-bg" ref="container">
      <div class="close theme-noborder" :style="lineStyle">
        <van-icon name="cross" @click="showDateSelector = false" :color="$config.blackTheme ? '#EAD092' : '#262626'" />
      </div>
        ...
        <div style="min-height:100%" ref="scrollArea">
          ...
          >
            <div class="clearfix">
              ...
              <div
                class="btn theme-color theme-border-1px theme-color-yellow-2 theme-border-yellow"
                @click="saveEdit"
              >{{editState ? '完成' : '编辑'}}</div>
            </div>
            <div class="collect">
              <div class="collect-list clearfix" v-if="selfCollectList && selfCollectList.length">
                ...
                >
                  ...
                  >
                    ...
                    <img :src="idConfig[gameSortV4Note.data[item.id]['series']].blackFirmIcon" class="firm" v-if="$config.blackTheme && gameSortV4Note.data && gameSortV4Note.data[item.id] && gameSortV4Note.data[item.id]['series'] && idConfig[gameSortV4Note.data[item.id]['series']]">
                    <img
                      ...
                      class="firm"
                      v-if="!$config.blackTheme && gameSortV4Note.data && gameSortV4Note.data[item.id] && gameSortV4Note.data[item.id]['series'] && idConfig[gameSortV4Note.data[item.id]['series']]"
                    />
                  </div>
                </vue-draggable>
              </div>
              <div class="no-collect" v-else>
                <div v-if="userInfo && JSON.stringify(userInfo) !== '{}'" class="no-collect-area">
                  <img src="@src/assets/img/game/new-game/no-collect-bg.png" />
                  <p class="no-collect-text theme-color-white">请点击官方推荐添加游戏</p>
                </div>
                <div v-else class="no-userInfo-area">
                  <img src="@src/assets/img/game/new-game/no-userInfo-img.png" />
          ...
          >
            ...
            <div class="recommend-list" v-if="showRecommendList.length">
              ...
              >
                <van-icon name="plus" class="icon" :color="$config.blackTheme ? '#fff' : '#222'" />
                <div class="cont theme-color-white">{{item.name}}</div>
                <img :src="idConfig[gameSortV4Note.data[item.id]['series']].blackFirmIcon" class="firm" v-if="$config.blackTheme && gameSortV4Note.data && gameSortV4Note.data[item.id] && gameSortV4Note.data[item.id]['series'] && idConfig[gameSortV4Note.data[item.id]['series']]">
                <img
                  :src="$config.home !== 'vns' ? idConfig[gameSortV4Note.data[item.id]['series']].AfirmIcon : idConfig[gameSortV4Note.data[item.id]['series']].BfirmIcon"
