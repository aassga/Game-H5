<template>
  <view-page style="background-color:#ffffff" header-touch full-box :noScroll="true" ref="viewPage" class="theme-header-bg">
    <title-header
      slot="header"
      class="header-top"
      v-no-touch-move
    ...
    >
      <div :class="[{FocusSearch:isFocusSearch},'top-bar1','theme-main-bg']">
        <span class="header-back icon iconfont h5-icon-left-menu-ico theme-color-yellow-1" @click="$router.push('/')"></span>
      ...
      <div :class="[{FocusSearch:isFocusSearch},'top-bar2','theme-main-bg']" @click="changeIsFocus(true)">
        <span
          class="header-back icon iconfont h5-icon-left-menu-ico theme-color-yellow-1"
        ...
        <van-search
          ...
          :clearable="false"
          :class="{'black-search' : $config.blackTheme}"
        />
        <img
      ...
      <div class="gray-bar theme-header-bg"></div>
    </title-header>
    <div :class="['content','newSolt','theme-header-bg',{'black-content':$config.blackTheme}]" slot="content" ref="content">
      <div v-show="!isFocusSearch">
        ...
        >
          <div class="fl left-nav" ref="leftNav" v-if="leftListArr.length > 0">
            ...
            >
              <div
                slot-scope="props"
                :class="[current === props.index ? 'theme-agent-tab-active theme-yellow-bg-2' : '' ,'borderRadius']"
                @click="changeHandler(props.index)"
              >
                <img :src="leftNavList[props.index].icon" alt />
                <div class="item-area theme-noborder">
                  <div :class="['game-name','theme-color-white',{'theme-color-black' : current === props.index}]">{{ leftNavList[props.index].name}}</div>
                </div>
              </div>
            </cube-scroll-nav-bar>
            <div class="bottomIcon theme-yellow-bottom-shadow">
              <i class="icon iconfont h5-icon-xiala- iconColor theme-color-yellow"></i>
            </div>
          </div>
          <div class="fl right-game" ref="rightGame">
            <van-tabs
              ...
              :border="false"
              :line-height="$config.blackTheme ? 0 : 2.5"
            >
              <van-tab v-for="(item,i) in typeList" :title="item.name" :key="i" style="background:red"></van-tab>
            </van-tabs>
            <div
              class="top"
            ...
            <scroll-area
              ...
              :onDownMore="true"
            >
              <div class="right-game-scroll theme-header-bg">
                ...
                <div class="animation-box" ref="animationBox" v-if="isShow">
                  <ul class="clearfix game-list" v-show="showGameList && showGameList.length > 0">
                    <li
                      :class="[{'chess':typeId === '10004'},'fl','theme-main-bg-5']"
                    ...
                    >
                      ...
                      <div v-else @click="clickGameHandle(per)">
                        <img
                          ...
                          v-if="gameSortV4Note.data && idConfig[gameSortV4Note.data[per.id]['series']]"
                        />
                        <img
                          class="error-img"
                      ...
                      <span
                        ...
                        v-if="gameSortV4Note.data && gameSortV4Note.data[per.id] && gameSortV4Note.data[per.id]['series'] && idConfig[gameSortV4Note.data[per.id]['series']] && !$config.blackTheme"
                      >{{ idConfig[gameSortV4Note.data[per.id]['series']].name}}</span>
                      <span
                        class="span-2"
                        v-if="typeId !== '10004' && per.payline && per.payline !== '0' && !$config.blackTheme"
                      >{{Number(per.payline) ? per.payline + '线' : per.payline}}</span>
                      <img :src="idConfig[gameSortV4Note.data[per.id]['series']].blackFirmIcon" class="firm-img" v-if="gameSortV4Note.data && gameSortV4Note.data[per.id] && gameSortV4Note.data[per.id]['series'] && idConfig[gameSortV4Note.data[per.id]['series']] && $config.blackTheme">
                      <span class="black-line" v-if="typeId !== '10004'  && per.payline && per.payline !== '0' && $config.blackTheme">
                        {{Number(per.payline) ? per.payline + '线' : per.payline}}
                      </span>
                      <div class="game-bottom">
                        <span class="game-name theme-color-gray-6">{{ per.name }}</span>
                        <div class="game-collect" @click.capture.stop="collectHandle(per)" v-if="$config.blackTheme">
                          <img
                            src="../../../assets/img/game/new-game/black_heart_red.png"
                            v-if="!per.isCollect"
                          />
                          <img src="../../../assets/img/game/new-game/black_heart.png" v-else />
                        </div>
                        <div class="game-collect" @click.capture.stop="collectHandle(per)" v-else>
                          <img
        ...
        <span :class="[{showReturnTop:showReturnTop},'returnTop','theme-bg','theme-yellow-bg-1']" @click="returnTop"></span>
      </div>
        ...
        <div style="height: 100%">
          <div class="search-hint" v-if="!searchGame.trim()">
            ...
            <div class="hot" style="height:auto">
              <div class="gray-bar" v-if="historylist.length && !$config.blackTheme"></div>
              <div class="title theme-color-white">
                热搜榜
                <img src="@src/assets/img/game/new-game/hot-search.png" class="hot-search" />
              </div>
              <div class="list theme-border-color" v-if="hotGameList && hotGameList.length">
                <div
                  class="hot-item theme-border-color"
                  v-for="(item,i) in hotGameList"
                  :key="i"
                  @click="changeSearchGame(item.name,true)"
                >
                  <div class="hot-name theme-border-color theme-color-white">{{item.name}}</div>
                </div>
              </div>
          ...
          <div v-else>
            <div class="search-box" ref="searchBox" v-if="!isLoding">
              ...
              >
                <li class="fl theme-main-bg-5" v-for="(per, index) in showSearchGameList" :key="index">
                  <div v-if="per.imgExists">
                    <img
                      class="game-list-img"
                      :src="imgUrl(per.icon)"
                  ...
                  <div class="game-bottom">
                    <span class="game-name theme-color-gray-6">{{ per.name }}</span>
                    <div class="game-collect" @click.capture.stop="collectHandle(per)" v-if="$config.blackTheme">
                      <img src="../../../assets/img/game/new-game/black_heart_red.png" v-if="!per.isCollect" />
                      <img src="../../../assets/img/game/new-game/black_heart.png" v-else />
                    </div>
                    <div class="game-collect" @click.capture.stop="collectHandle(per)" v-else>
...
export default {
  ...
  created() {
    // safari 谷歌浏览器频繁进入页面报错
    if(this.$mobileDevice.isSafari || (/AppleWebKit/.test(navigator.userAgent) && /Safari/.test(navigator.userAgent))) {
    ...
    if(this.userInfo && JSON.stringify(this.userInfo) !== '{}') {
      ...
      if(!localStorage[`${this.typeId}leftNavList`]) {
        this.getData(true);
      }else {
  ...
  beforeDestroy() {
    clearTimeout(this.gameNumberTimer)
  }
};
</script>
  ...
  .game-list {
    ...
    li {
      ...
      .black-line {
        ...
        text-align: center;
        color: #fff;
        background-image: url('../../../assets/img/game/new-game/line-bg.png');
