<template>
  <div>
    <scroll-selector :class="['scroll-selector',{'black-scroll-selector' :$config.blackTheme || $config.newQP,'blush-scroll-selector' :$config.blush} ]" v-model="index" :list="['百位走势', '十位走势', '个位走势']"></scroll-selector>
    <van-tabs v-model="index" class="lottery-trend no-tab" animated swipeable- :lazy-render="false">
      <van-tab v-for="ballPanel in 3" :key="ballPanel">
        <div class="lottery-trend-body">
          ...
          <scroll-box class="lottery-trend-content" direction="horizontal">
            <ul>
              <li class="row title" ref="title">
                <ul class="row-content">
                  <li class="col col-result">
                    <span class="col-content">开奖号码</span>
                  </li>
                  <li class="col col-num" v-for="n in 10" :key="n">
