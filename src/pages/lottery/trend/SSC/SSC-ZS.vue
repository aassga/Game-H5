<template>
  <div>
    <scroll-selector :class="['scroll-selector',{'black-scroll-selector' :$config.blackTheme || $config.newQP,'blush-scroll-selector' :$config.blush} ]" v-model="index" :list="['万位走势','千位走势','百位走势','十位走势','个位走势']"></scroll-selector>
      ...
      <van-tab v-for="ballPanel in 5" :key="ballPanel">
        <div class="lottery-trend-body">
          <ul class="lottery-trend-issue">
            <li class="row title issue theme-main-bg-5 theme-border-color">
            ...
            <li class="row issue theme-main-bg-5 theme-border-color" v-for="(item, i) in list" :key="i">
              <ul class="row-content">
                <li class="col col-issue">
          ...
          <scroll-box class="lottery-trend-content" direction="horizontal">
            <ul>
              <li class="row title theme-main-bg-5 theme-border-color" ref="title">
                <ul class="row-content">
                  <li class="col col-result theme-main-bg-5 theme-border-color">
                    <span class="col-content ">开奖号码</span>
                  </li>
                  <li class="col col-num theme-main-bg-5 theme-border-color" v-for="n in 10" :key="n">
                    <span class="col-content">{{n-1}}</span>
                  </li>
                </ul>
              </li>
              <li class="row theme-main-bg-5 theme-border-color" v-for="(item, i) in list" :key="i">
                <ul class="row-content">
                  <li class="col col-result theme-main-bg-5 theme-border-color">
                  ...
                  <li class="col col-num theme-main-bg-5 theme-border-color" v-for="n in 10" :key="n">
                  <span :ref="'ball-' + (ballPanel - 1) + '-' + i" class="col-content" v-if="(n-1)===item.resultNum[ballPanel - 1]">
