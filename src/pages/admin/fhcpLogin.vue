  ...
  <view-page class="login-box theme-header-bg" :full-box="!isShowFixed" ref="view-page">
    <div slot="content">
      <div class="content" v-show="!showStep">
        ...
        <div class="head">
          <img class="img-bg" :src='"../../assets/img/login/" + $config.name + "/login-bg.png"'/>
        </div>
        <div class="title" :class="{'js85-title':$config.name === 'js85'}">
          <span @click="goLogin" class="theme-main-bg-3" :class="tabIndex ===0?'active':'theme-color-white'">登录</span>
          <span @click="goRegister" class="theme-main-bg-3" :class="tabIndex ===1?'active':'theme-color-white'">注册</span>
        </div>
        ...
        <div class="down-shade" :class="{'js85-down-shade':$config.name === 'js85'}" v-if="isShade">
        <div class="down-box">
...
export default {
  ...
  created() {
    ...
    if(!['hty','dqr','839qp','103qp','fhcp','aqvns','dalao','xpj83','js85'].includes(this.$config.name)) {
      this.goRegister()
    }
    this.checkArchCode();
...
.content {
  ...
  .js85-title{
    padding: 3.6rem 0.24rem 0 0.24rem !important;
  }
  .title {
