<template>
  <view-page v-if="isValid" class="body" full-box load-more ref="view-page" @load-top="onPullingDown" @load-down="onPullingUp">
    <title-header :title="!bet?'视讯投注列表':'视讯投注详情'" class="header" slot="header" @back="back" :back-event="true">
    ...
    <van-popup slot="static" v-model="showFilter" position="bottom" get-container="body" :lazy-render="false">
      <van-picker show-toolbar :item-height="40" v-model="showFilter" :columns="gameList" value-key="name" title="选择游戏" :confirm-button-text="'确定'"
  ...
  methods: {
    ...
    getBetItem (item) {
      if (item.time) {
        ...
        return ['和', '已中奖', '未中奖'][this.bet[item.k]]
      } else {
