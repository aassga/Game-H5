<template>
	<view-page class="agent-bet theme-header-bg" ref="view-page" full-box @scroll-height-change="scrollHeight=$event" no-scroll>
		<title-header class="theme-main-bg theme-header-bottom" title="投注记录" slot="header" backEvent @back="goback()">
	    ...
	    <div :class="['theme-header-bg',{'black-tab' : $config.blackTheme || $config.newQP,'blush-tab':$config.blush}]" v-if="dataType === 'day'" slot="header-static" style="padding: 0.2rem 0 0 0;">
	    ...
	    <div class="record-content" slot="content">
	    	<transition-group :name="animation" class="animation-box" tag="div" ref="animationBox">
	    		<component :userId="userId" :dataType="dataType" :key="'bet_list_'+parentTabIndex" v-bind:is="targetComponent" :parentTabIndex="parentTabIndex" :scrollHeight="scrollHeight"></component>
	    	</transition-group>
	    </div>
	    <div slot="static">
	        <agency-date-selector :newQP="$config.blackTheme || $config.newQP" :isZs="isZs" v-model="date" :default-index="defaultIndex || 0" :show.sync="showDateSelector" @okSelect="okSelect"></agency-date-selector>
	    </div>
	</view-page>
</template>
...
export default {
	...
	created(){
		if(localStorage['AgentBetTempSave']){
			var tempSave = JSON.parse(localStorage['AgentBetTempSave']);
			this.date = tempSave.date;
			this.zsType = tempSave.zsType;
			this.dataType = tempSave.dataType;
			this.parentTabIndex = tempSave.parentTabIndex;
			this.defaultIndex = tempSave.defaultIndex;
			this.targetComponent = tempSave.targetComponent;
		}else{
            //从财务报表-团队盈利-进入投注记录查询参数处理
            if(this.$route.query.selectType){
                this.date = JSON.parse(this.$route.query.date);
                this.zsType = 0;
                this.defaultIndex = Number(this.$route.query.defaultIndex);
                this.dataType = this.$route.query.selectType;
                this.parentTabIndex = Number(this.$route.query.parentTabIndex);
                this.targetComponent = this.$route.query.targetComponent;
                if(this.$route.query.uid){
                    this.userId = Number(this.$route.query.uid);
                }
            }
        }
	...
	methods:{
		...
		localSave(){//将页面信息存放到本地缓存
			var tempSave = {
				...
				defaultIndex: this.defaultIndex, //日期下标
				targetComponent: this.targetComponent //存放组件名称
			}
			
			localStorage['AgentBetTempSave'] = JSON.stringify(tempSave);
		},
		initData(){
			var item = {
				date: this.date,
	    ...
	    goback(){
			...
			delete localStorage.AgentBetTempSave
			this.$router.back();
	    }
	},
	beforeDestroy(){
