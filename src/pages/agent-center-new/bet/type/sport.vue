                                ...
                                <li class="recode-item theme-after-border-color" @click="onItemClick(item)" v-for="(item, k) in list" :key="k">
                                    <div class="recode-item-col">
                                        ...
                                        <p class="info-row">
                                            ...
                                            <span class="bet theme-color-gray-2">投注:{{item['betAmount']}}</span>
                                        </p>
                                        <p class="info-row">
                                    ...
                                    <div class="recode-item-col">
                                        ...
                                        <p class="commission">
                                            ...
                                            <span class="status-ball icon iconfont" v-else-if="item.status==4 || item.status==0 || item.status==3" :class="statusOther[item['status']].class"></span>
                                        </p>
                                        <span v-if="item.status==1 || item.status==2" class="status-ball icon iconfont" :class="statusOther[item['status']].class">
...
export default {
    ...
    mounted(){
        var _this = this;
        this.$root.$off("SportLoadEvent").$on("SportLoadEvent",item=>{
            //彩票数据加载事件发布
    ...
    methods:{
        ...
        onItemClick(item){
            ...
            this.$router.push('/user/agency-center-new/recode/info/SPORT');
        },
        onRefresh(){
            //下拉刷新数据
            this.totalCount = 0;
