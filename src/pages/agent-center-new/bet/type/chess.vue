                            ...
                            <ul style="margin-top: 0;" v-if="list.length">
                                <li class="recode-item theme-after-border-color" @click="onItemClick(item)" v-for="(item, k) in list" :key="k">
                                    <div class="recode-item-col">
                                        ...
                                        <p class="info-row">
                                            <span class="bet theme-color-gray-2" style="overflow: hidden; white-space: nowrap;text-overflow: ellipsis;color: #888888;font-size: 0.26rem;">{{ getBillNo(item.platform)+'('+item.billNo+')' }}</span>
                                        </p>
                                    </div>
                                    <div class="recode-item-col">
                                        <p class="commission">
...
export default {
    ...
    mounted(){
        var _this = this;
        this.$root.$off("ChessLoadEvent").$on("ChessLoadEvent",item=>{
            //彩票数据加载事件发布
    ...
    methods:{
        ...
        onItemClick(item){
            ...
            this.$router.push('/user/agency-center-new/recode/info/CHESS');
        },
        onRefresh(){
            //下拉刷新数据
            this.totalCount = 0;
