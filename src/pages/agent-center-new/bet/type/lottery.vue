<template>
	<div class="content-body">
		<div class="tab-view" v-if="dataType !== 'all'">
			<div class="view-box search-row theme-header-bg" :class="{'theme-border-1px':$config.blackTheme || $config.newQP,'theme-border-white':$config.blush}">
                <span class="search-icon icon iconfont h5-icon-sousuotubiao theme-color theme-color-yellow"></span>
                <van-field v-model="username" :class="['input theme-header-bg theme-color-white',{'black-input' : $config.blackTheme || $config.newQP ||$config.blush}]" type="text" placeholder="输入帐号可进行筛选" clearable @blur="onSearch"></van-field>
            </div>
		</div>
        <ul class="type-tab-list theme-main-bg-5 theme-after-border-color" v-if="dataType === 'all'">
        	<!--7日以上数据表头-->
            <li class="type-tab-item theme-color-white" v-for="(type, j) in dayTitle" :key="j">
                {{type.name}}
            </li>
        </ul>
        <ul class="type-tab-list theme-main-bg-5 theme-after-border-color" v-if="dataType === 'day'">
            <li class="type-tab-item" :class="[{'active theme-bg theme-yellow-bg-2 theme-color-black':j===tabIndex},{'theme-color-white' :j!==tabIndex }]" v-for="(type, j) in lotteryTypes" @click="onTabChage(j)" :key="j">{{type.name}}
            </li>
        </ul>
        <div class="item-list">
        	<transition-group :name="animationList" class="animation-list theme-main-bg-5" tag="p" ref="animationList">
        		<div class="list-content" :key="'lottery_all_wap_'+tabIndex" v-if="dataType === 'all'">
        			<!--7日以上数据内容-->
            				...
            				<ul style="margin-top: 0;" v-if="list.length">
	            				<li class="recode-item theme-after-border-color" @click="onItemClick(item)" v-for="(item, k) in list" :key="k">
	            					<div class="recode-item-col">
	                                    ...
	                                    <p class="info-row">
	                                    	<span class="ball">
	                                            <i class="theme-color-yellow" style="display:inline-block;">{{item['playwayName']}}</i>
	                                            <i>{{item.ball?':':''}}</i>
	                                            <i class="theme-color-yellow">{{item.ball||''}}</i>
	                                            <i>{{item.ball?'-':':'}}</i>
	                                            <i class="code">{{item['betInfo']}}</i>
	                                        </span>
	                                    </p>
	                                    <p class="info-row">
	                                    	<span class="bet theme-color-gray-2" style="margin-right:0.24rem"><span>{{ item.lotteryName }}</span>{{'(第'+forMartIssu(item.issue)+'期)'}}</span>
	            					...
	            					<div class="recode-item-col">
	                                    ...
	                                    <p class="commission">
	                                        ...
	                                        <span class="status-ball icon iconfont" v-else-if="item.status==4 || item.status==0 || item.status==5" :class="statusLottery[item['status']].class"></span>
                                        </p>

	                                    <span v-if="item.status==1 || item.status==2 || item.status==3 " class="status-ball icon iconfont" :class="statusLottery[item['status']].class"></span>
	            					    <span v-else class="status-ball"></span>

                                    </div>
	            				</li>
	            			</ul>
	            			<div :style="{height:(scrollHeight-(otherHeight*rem))+'px'}" v-else>
	                            <img style="width: 100%;margin-top: 1rem;" :src="noDataImg" alt />
	                            <p class="theme-color-white" style="font-size: 0.3rem;color: #666666">暂无数据</p>
	                        </div>
...
export default {
	...
	mounted(){
		var _this = this;
		this.$root.$off("LotteryLoadEvent").$on("LotteryLoadEvent",item=>{
			//彩票数据加载事件发布
	...
	methods:{
		...
		onItemClick(item){
			this.$store.state.agency.recodeItem = item
      		this.$router.push('/user/agency-center-new/recode/info/LOTTERY');
		},
		onRefresh(){
            this.totalCount = 0;
            this.page = 1;
		...
		loadMore(status) {
			...
			const data = {
                ...
                status:this.lotteryTypes[this.tabIndex].status
            }
            if(!!this.username){
